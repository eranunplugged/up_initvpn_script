...
docker run -v $OVPN_DATA:/etc/openvpn --log-driver=none --rm ghcr.io/eranunplugged/up_openvpn_xor:${OVPN_IMAGE_VERSION} ovpn_genconfig -u tcp://${PUBLIC_IP}:443
sed -i 's/1194/443/i' /var/lib/docker/volumes/${OVPN_DATA}/_data/openvpn.conf
docker run -v $OVPN_DATA:/etc/openvpn -d -p 443:443/tcp --cap-add=NET_ADMIN --name ovpn ghcr.io/eranunplugged/up_openvpn_xor:${OVPN_IMAGE_VERSION}
...
for i in $(seq 1 $NUM_USERS); do
  docker run -v $OVPN_DATA:/etc/openvpn --log-driver=none --rm -e DEBUG=1 ghcr.io/eranunplugged/up_openvpn_xor:${OVPN_IMAGE_VERSION} ovpn_getclient "user$i" > "user$i.ovpn"
done
docker stop ovpn
