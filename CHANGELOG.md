...
- UNP-8203: tag every `up_openvpn_xor` image pull with `:${OVPN_IMAGE_VERSION}` in `init-vpn.sh` and `ovpn-gen-peers.sh` (5 + 1 pulls were untagged, i.e. `:latest`). The OpenVPN server image version is now controlled solely by the Vault key, matching the WireGuard/reality pattern.
